...
class ZipF
{
     ...
     * @var [type]
     */
    private $FILENAME = "./ComFile/";
    /**
     * 压缩文件的目录
     */
    private $COMPRESS = "./result/";
    ...
    function zip() {
        ...
        $time = time();
        $time = date('Y-m-d', $time); //最终生成的文件名
        
        //--------------
        //如果不指定目录，下载文件会生成在当前目录下
        //--------------
        //压缩文件的文件名，同时也是生成压缩文件的目录
        ...
        if ($zip->open($filename, ZipArchive::CREATE) === TRUE) {
            ...
            $this->addFileToZip($this->COMPRESS, $zip);
            $zip->close();
        }
     ...
     */
    private function addFileToZip($path, $zip, $folder = "") {
        //打开当前文件夹由$path指定。
        ...
        while (($filename = readdir($handler)) !== false) {
            //文件夹文件名字为'.'和‘..’/readme.md，不要对他们进行操作
            if ($filename != "." && $filename != ".." && $filename != "Readme.md") {
                // 如果读取的某个对象是文件夹，则递归
